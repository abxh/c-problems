...
static int histogram_helper(void* ctx_untyped) {
    ...
    for (uint64_t i = begin; i < end; i++) {
        ...
        if (0 <= bin_index && bin_index < (int64_t)MIN(ctx->args.num_bins, INT64_MAX)) {
            uint64_t x_old = atomic_load(&ctx->bin_cnts[bin_index]);
            while (!atomic_compare_exchange_weak(&ctx->bin_cnts[bin_index], &x_old, x_old + 1));
        }
    }
...
static int output_bins_helper(void* ctx_untyped) {
    ...
    for (uint64_t i = begin; i < end; i++) {
        ctx->args.bins_out[i] = atomic_load(&ctx->bin_cnts[i]);
    }

    return 0;
}

void histogram_par_atomic(const size_t num_bins, uint64_t* bins, const size_t num_indices, const int64_t* indices) {
